...
async function run() {
    ...
    for (const question of step.questions) {
      ...
      if (answer) {
        if (question.name === "REPO_ADMIN_ACCESS_TOKEN") {
          const didSucceed = await testAccessToken(answer);
          if (!didSucceed) {
        ...
        if (question.name === "APP_PRIVATE_KEY" || question.name === "KERNEL_PUBLIC_KEY") {
  ...
  for (const [key, value] of Object.entries(parsedEnv)) {
    ...
    await storeSecret(
      key,
      value,
      answers["Repository settings"]["TELEGRAM_BOT_REPOSITORY_FULL_NAME"] as string,
      answers["Secret upload"]["REPO_ADMIN_ACCESS_TOKEN"] as string
    );
  }
