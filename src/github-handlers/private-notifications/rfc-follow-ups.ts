...
import { retrieveUsersByGithubUsernames } from "./helpers";
import { NotificationMessage } from "./notification-messages";
import { CommentTriggerBase } from "./comment-trigger-base";

export class RfcCommentHandler extends CommentTriggerBase {
  ...
  private async _sendRfcNotification(rfcComment: RfcComment, githubUsername: string, telegramId: string | number): Promise<void> {
    ...
    await this._deliverNotification(telegramId, rfcMessage);
  }
  ...
  private _updateUserRfcComments(fetchedUsers: StorageUser[]) {
    ...
    const rfcComment: RfcComment = {
      ...
      created_at: this.context.payload.comment.created_at,
      updated_at: this.context.payload.comment.updated_at,
      comment_url: this.context.payload.comment.html_url,
    };
